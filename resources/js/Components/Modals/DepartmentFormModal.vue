            ...
            <div class="bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden max-w-md w-full z-10 transform transition-all flex flex-col">
                ...
                <form @submit.prevent="$emit('submit')" class="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        />
                        <div v-if="form.errors.short_name" class="text-rose-500 text-xs mt-1">{{ form.errors.short_name }}</div>
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Department Head</label>
                        <select v-model="form.department_head_id" class="w-full rounded-lg border-slate-200 shadow-sm focus:border-[#0D9488] focus:ring-[#0D9488] text-sm">
                            <option value="">Unassigned</option>
                            <option v-for="m in membersList" :key="m.id" :value="m.id">
                                {{ m.name }} ({{ m.role }})
                            </option>
                        </select>
                        <div v-if="form.errors.department_head_id" class="text-rose-500 text-xs mt-1">{{ form.errors.department_head_id }}</div>
                    </div>
